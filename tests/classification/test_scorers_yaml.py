...
def _hardcoded_reference_scorers() -> ScorerSpec:
    ...
    return {
        "plaat": [
            ...
            (
                ("unfoldable", "seamed_tube"),
                lambda v: 1.0 if (v[0] and not v[1]) else 0.0,
                1.2,
            ),
            (
                ("has_bends", "seamed_tube"),
                lambda v: 1.0 if (v[0] and not v[1]) else 0.0,
                1.2,
            ),
...
_PROBE_FEATURES: list[dict] = [
    ...
    {  # seamed hollow section - unfolds with bends but is a profile.
        # Exercises the and_not cross-terms: the plaat unfold rewards must be
        # gated off and the profiel seamed_tube reward must fire.
        "top1_face_pct": 0.16,
        "unfoldable": True,
        "has_bends": True,
        "seamed_tube": True,
